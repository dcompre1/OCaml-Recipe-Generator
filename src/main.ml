...
let rand = 
  ...
  Command.Let_syntax.(
    let%map_open filename = anon (maybe_with_default "my_recipe.txt"("filename" %: string))  in fun () -> G.rand_meal filename)(*fun () -> print_endline filename) *)
...
let name = 
  ...
  ( 
    ...
    in fun () -> print_endline meal_name; print_endline filename) 
...
let () = let p = prompt_for_command in 

...
let get_meal (m : meal list) : meal =
  ...
  | m :: [] -> m
  | m ->
      let n = Random.int (List.length m - 1) in
      List.nth_exn m n
...
let is_file (filename : string) : string =
   if
   String.is_suffix ~suffix:".txt" filename then filename else filename ^ ".txt"
...
let rec prompt_for_recipe (m : meal) (l : meal list) (filename : string) : unit
  ...
  | l -> (
      ...
      | "yes" | "YES" | "y" ->
          ...
          prompt_for_recipe (get_meal new_list) new_list filename
      | "no" | "NO" | "n" ->
         let filename = 
         match filename with
...
let get_recipe (c : char) (filename : string) (input : string)
   ...
   if String.is_empty input then print_endline "No input given" else
  let meals =
    match c with
    | 'i' ->
        find_meals
  ...
  let meals = filter_meals restrictions meals in
   if no_meals meals then print_endline "Sorry, there are no recipes that fit your criteria" else
   let meal = get_meal meals in
...
let rec prompt_for_recipe2 (l : meal list) (filename : string) : unit =
  ...
  | l -> (
      ...
      let input1 =
        printf
          "Would you like to view any of these recipes? Please type the number \
           next to the recipe you would like to view, or simply type \"no\": \
           %!";
        match In_channel.input_line In_channel.stdin with
        | None -> failwith "no value entered. aborting."
        | Some line -> line
      ...
      | i -> (
         ...
         in
          ...
          | _ -> 
            ...
            | f -> is_file f
            in
              print_endline ("Your recipe has been saved in the file" ^ filename);
              M.meal_to_file meal filename))
...
let get_recipe_list (c : char) (filename : string) (input : string)
  ...
  in
  let meals = filter_meals restrictions meals in
  prompt_for_recipe2 meals filename
